...
public class PluginInterceptActivity extends FragmentActivity {
    ...
    public View onCreateView(View parent, String name, Context context, AttributeSet attrs) {
        ...
        try {
            ...
            if (mUseCompatTheme && Build.VERSION.SDK_INT < Build.VERSION_CODES.LOLLIPOP) {
                viewCls = mContentProxy.getClassLoader().loadClass(getCompatV7ViewClass(name));
            } else {
                viewCls = mContentProxy.getClassLoader().loadClass(name);
            }
            if (null != viewCls) {
    ...
    private boolean useAppCompatTheme() {
        try {
            Class styleCls = mContentProxy.getClassLoader().loadClass("android.support.v7.appcompat.R$styleable");
    ...
    public Intent getIntent() {
        if (mContentProxy.getContext() instanceof ActivityHostProxy) {
            if (null == mIntent) {
                ...
                final DexClassLoader pluginClassLoader = mContentProxy.getClassLoader();
